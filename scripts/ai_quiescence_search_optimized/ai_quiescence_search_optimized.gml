/// @function ai_quiescence_search_optimized(alpha, beta, maximizing_player, depth_)
/// @param {real} alpha Alpha value
/// @param {real} beta Beta value
/// @param {bool} maximizing_player Whether maximizing
/// @param {real} depth_ Remaining quiescence depth_
/// @returns {real} Evaluation score_
...
function ai_quiescence_search_optimized(alpha, beta, maximizing_player, depth_) {
    ...
    capture_moves = ai_order_captures(capture_moves);
    
    if (maximizing_player) {
        for (var i = 0; i < array_length(capture_moves) && !search_cancelled; i++) {
            var move = capture_moves[i];
            var game_state = ai_save_game_state();
            
            ai_make_move_simulation(move);
            var score_ = ai_quiescence_search_optimized(alpha, beta, false, depth_ - 1);
        ...
        for (var i = 0; i < array_length(capture_moves) && !search_cancelled; i++) {
            ...
            ai_make_move_simulation(move);
            var score_ = ai_quiescence_search_optimized(alpha, beta, true, depth_ - 1);
            ai_restore_game_state(game_state);
