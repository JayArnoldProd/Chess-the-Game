...
if (set_appearance == false && instance_exists(Board_Manager) && instance_exists(Object_Manager)) {
    ...
    for (var i = 0; i < array_length(Board_Manager.special_tile_textures); i++) {
        ...
        if (override[0] != "row" && override[0] != "column") {
            ...
            if (grid_x == col && grid_y == row) {
                ...
                var board_val = color_get_value(override[2]);//color_get_value(Board_Manager.white_color);
    ...
    if (!override_found) {
		...
		if (tile_type == 0) {
		    if !((image_index>=Board_Manager.white_textures[0]) && (image_index<=Board_Manager.white_textures[array_length(Board_Manager.white_textures)-1])) {
		...
		switch (tile_type) {
		    case 0: //normal
		        ...
		        image_alpha = Board_Manager.white_alpha;
		        break;
		    case 1: //water
