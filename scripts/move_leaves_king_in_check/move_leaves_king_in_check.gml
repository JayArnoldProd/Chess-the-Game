...
function move_leaves_king_in_check(piece, target_x, target_y) {
    ...
    // Check if any enemy piece attacks the king DYNAMICALLY (not using cached valid_moves)
    var in_check = false;
    ...
    with (Chess_Piece_Obj) {
        ...
        if (piece_id == "pawn") {
            ...
            var pawn_dir = (piece_type == 0) ? -1 : 1; // white=-1(up), black=+1(down)
            if (abs_dx == 1 && dy == pawn_dir) {
                in_check = true;
            }
        }
        else if (piece_id == "knight") {
            // Knights jump: L-shape
            ...
            if (abs_dx == abs_dy && abs_dx > 0) {
                ...
                for (var dist = 1; dist < abs_dx; dist++) {
                    ...
                    // Check if any piece blocks this square
                    var blocker = noone;
                    with (Chess_Piece_Obj) {
            ...
            // Queen: diagonal OR straight ray-cast
